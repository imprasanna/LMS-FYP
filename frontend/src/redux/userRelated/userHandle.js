...
export const getUserDetails = (id, address) => async (dispatch) => {
  ...
  try {
    ...
    dispatch(
      getError(error.response?.data?.message || "Failed to fetch user details.")
    );
  }
};
...
export const updateUser = (fields, id, address) => async (dispatch) => {
  ...
  try {
    ...
    dispatch(
      getError(error.response?.data?.message || "Failed to update user.")
    );
  }
};
...
export const addStuff = (fields, address) => async (dispatch) => {
  ...
  if (!address) {
    console.error(
      "addStuff Error: address is undefined! Please pass the correct address."
    );
    dispatch(authError("Invalid API endpoint: Address is missing."));
    return;
  }
