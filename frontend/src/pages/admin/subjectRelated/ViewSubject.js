...
const ViewSubject = () => {
  ...
  useEffect(() => {
    if (!classID || !subjectID) {
      console.error("Missing classID or subjectID in URL.");
      navigate("/Admin/subjects"); // Redirect to a safe fallback page
      return;
    }

    dispatch(getSubjectDetails(subjectID, "Subject"));
    dispatch(getClassStudents(classID));
  }, [dispatch, subjectID, classID, navigate]);

  if (error) {
    console.error("Error loading subject details or students:", error);
  }
  ...
  const SubjectDetailsSection = () => (
    <>
      ...
      <Typography variant="h6" gutterBottom>
        Study Material Link: {subjectDetails?.subCode || "N/A"}
      </Typography>
      <Typography variant="h6" gutterBottom>
